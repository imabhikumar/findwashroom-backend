...
// Attach service units to booking
    ...
    public function cancel(int $customerId, int $bookingId): Booking
    {
        return DB::transaction(function () use ($customerId, $bookingId) {
            ...
            if ($booking->status !== 'pending') {
                throw new BadRequestHttpException('Only pending bookings can be cancelled by the customer.');
            }
            if (($booking->payment_status ?? 'unpaid') === 'paid') {
                throw new BadRequestHttpException('Paid bookings cannot be cancelled from this flow.');
            }
            ...
            // Any unpaid pending gateway attempt is no longer retryable after booking cancellation.
            if ($booking->payment) {
